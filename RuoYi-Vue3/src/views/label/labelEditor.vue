...
<template>
  <div class="app-container" ref="root">
    ...
    <pagination v-if="flag" v-show="total > 0" :total="total" v-model:page="queryParams.pageNum"
    ...
    <div style="margin-top: 20px; display: flex;">
      ...
      <el-table :data="times" :highlight-current-row="false" style="width: 100%" :row-class-name="tableRowClassName"
        @row-click="rowClick">
    ...
    <el-dialog :title="title" v-model="open" width="700px" append-to-body>
      <el-form ref="taskRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="音频波形">
          <div id="waveform-detail" class="waveform-container" v-if="form.audioFileName"
            style="width: 100%; height: 100px;"></div>
          <div v-else>请先上传音频文件</div>
        </el-form-item>
        <el-form-item label="任务状态" prop="status">
          <el-select v-model="form.status" placeholder="请选择任务状态">
            <el-option v-for="dict in task_status" :key="dict.value" :label="dict.label"
...
function renderDemoWaveform(
  audioFileName = '/profile/upload/2025/10/14/测试音频_20251014173656A001.mp3') {
  ...
  if (container) {
    ...
    ws = WaveSurfer.create({
      ...
      hideScrollbar: false,
      interact: true, // 可交互
      url: '/dev-api/profile/upload/2025/10/14/测试音频_20251014173656A001.mp3',
      plugins: [
        regions,
        timeline,
        hover,
    ...
    // 加载音频文件    
    // const audioUrl = getAudioUrl(audioFileName)
    // const audioUrl = getAudioUrl('/profile/upload/2025/10/14/测试音频_20251014173656A001.mp3')
    // ws.load(audioUrl)
