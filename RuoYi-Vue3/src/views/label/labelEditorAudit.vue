  ...
  <div class="app-container">
    ...
    <el-dialog  v-model="diffDialogVisible" :fullscreen="false" width="90%">
      <div style="display: flex; justify-content: space-around; line-height: 30px;">
        <!-- 左边选项 -->
        <el-dropdown size="large" @command="handleCommandLeft">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item v-for="vs in versions" :command="vs" :disabled="vs.value==rightTG.value">{{ vs.label }}</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
        <!-- 右边选项 -->
        <el-dropdown size="large" @command="handleCommandRight">
          <span class="el-dropdown-link">
            {{ rightTG.label }}
            <el-icon class="el-icon--right">
              <arrow-down />
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item v-for="vs in versions" :command="vs" :disabled="vs.value==leftTG.value">{{ vs.label }}</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
        <!-- <el-text class="mx-1" size="large">{{ $t('label.labelEditor.标注员的标注') }}</el-text>
        <el-text class="mx-1" size="large">{{ $t('label.labelEditor.审核员的标注') }}</el-text> -->
      </div>
...
const newText = ref('')
const versions = ref([
  {label:'原始的标注', value:'originalTextGrid'}, 
  {label:'标注员的标注', value:'textGrid'}, 
  {label:'审核员的标注', value:'auditTextGrid'}
])
...
const handleCommandLeft = (command => {
  leftTG.value=command
  showTextDiff()
})
const handleCommandRight = (command => {
  rightTG.value=command
  showTextDiff()
})
//显示标注前后文本对比
...
function loadTextGrid(){
  if(!task.data.textGrid){//如果标注员的标注数据为空，则提示
    proxy.$message.error(proxy.$t('label.labelEditor.缺少标注文本TextGrid'))
    return
  }
  if(!task.data.auditTextGrid){//如果审核员的标注数据为空，则复用标注员的
    task.data.auditTextGrid = task.data.textGrid
    console.log('载入【标注员】的标注数据')
  }else{
    console.log('载入【审核员】的标注数据')
  }
  // ----将预标注文本转为json---
  // console.log('task.textGrid-->\n',task.data.textGrid)
  task.textGridJson = parseTextGrid(task.data.auditTextGrid)
